#!/usr/bin/env python3
"""
Kashf — TPU v6e run-2: SEQ_LEN=4096, max_loop_iters=6, grad-ckpt enabled.

    PJRT_DEVICE=TPU python training/train_fineweb_v6e_run2.py

Differences from run-1 (train_fineweb_v6e.py):
  - SEQ_LEN      4096   (vs 2048)
  - GRAD_ACCUM   16     (vs 32)   — same 524 288 tok/step global batch
  - max_loop_iters 6    (vs 2)
  - USE_GRAD_CKPT True  — required to keep activation memory under HBM limit
  - CKPT_EVERY   100    (vs 500)
  - CKPT_DIR     kashf_checkpoints_run2
  - GCS prefix   fineweb-v6e-run2
"""
...
def save_checkpoint(model, optimizer, step: int, cfg, ckpt_dir: str, keep_last: int = 5):
...
def _train_fn(index: int, cli_args):
    ...
    if is_master:
        ...
        mprint(f"max_loop_iters: 6  |  grad_ckpt: {USE_GRAD_CKPT}")
...
def main():
    argparse.ArgumentParser(description="Kashf v6e run-2 training on FineWeb-Edu").parse_args()
