#!/usr/bin/env python3
"""
Kashf — pretraining on Wikipedia English, tuned for RTX 4070 8 GB VRAM.

    python training/train_wikipedia_4070.py

Changes vs train_wikipedia.py:
  - torch.compile for ~20-40% faster throughput
  - MICRO_BATCH 8 → 16, GRAD_ACCUM 8 → 4 (same effective batch)
  - TF32 matmul/cudnn enabled explicitly
  - num_workers=2 with prefetch (fall back to 0 if fds error)
  - TARGET_TOKENS 4B → 1B for a first local verification run

Dataset:  wikimedia/wikipedia  20231101.en  (~20 GB, ~4B tokens)
Hardware: RTX 4070 8 GB VRAM
Time:     ~6 hours for 1B tokens
"""
...
SEQ_LEN        = 512
MICRO_BATCH    = 16          # was 8 — doubled to better saturate the GPU
GRAD_ACCUM     = 4           # was 8 — halved so effective batch stays 32,768 tokens
LR             = 3e-4
MIN_LR         = 3e-5
WEIGHT_DECAY   = 0.1
WARMUP_STEPS   = 500
TARGET_TOKENS  = 1_000_000_000   # 1B — verify loss curve before scaling to 4B
LOG_EVERY      = 10
CKPT_EVERY     = 1000
CKPT_DIR       = os.environ.get("KASHF_CKPT_DIR", "kashf_checkpoints")
GRAD_CLIP      = 1.0
USE_GRAD_CKPT  = True        # keeps VRAM safe during activation spikes
...
def main():
    ...
    if device == "cuda":
        # Allow TF32 on matmuls and convolutions — free ~10% speedup, no quality loss
    ...
    # Compile — ~20-40% faster on Ada Lovelace, first step takes ~60s to compile
    if device == "cuda":
    ...
    # Optimizer — must be created after compile
    ...
    # Dataset — num_workers=2 overlaps tokenization with GPU compute.
    # If you see a "too many fds" crash, change num_workers to 0.
    dataset = WikipediaDataset(tok, SEQ_LEN)
    try:
        loader = DataLoader(dataset, batch_size=MICRO_BATCH, num_workers=2,
                            pin_memory=(device == "cuda"), prefetch_factor=2)
        # Probe the loader once to catch forkserver fd errors early
        _ = next(iter(loader))
        loader = DataLoader(dataset, batch_size=MICRO_BATCH, num_workers=2,
                            pin_memory=(device == "cuda"), prefetch_factor=2)
    except Exception:
        print("num_workers=2 failed — falling back to num_workers=0")
